...
# from candidate_model import BartForConditionalGeneration
...
def evaluation(args):
    ...
    test_set = BrioDataset("/scratch/users/sutd/nirmalen/cnndm/cnndm/diverse/test", args.model_type, is_test=True, max_len=512,
     is_sorted=False, max_num=args.max_num, is_untok=True, total_len=args.total_len, is_pegasus=args.is_pegasus)
    batch_size = 1
    dataloader = DataLoader(test_set, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
    # build models
    ...
    if args.do_reranking:
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, total=len(test_set) // batch_size):
                ...
                # print(output['score'].shape,output['summary_score'].shape)
...
def run(rank, args):
    ...
    train_set = BrioDataset("/scratch/users/sutd/nirmalen/cnndm/cnndm/diverse/train", args.model_type, max_len=args.max_len, max_num=args.max_num, total_len=args.total_len, is_pegasus=args.is_pegasus)
    val_set = BrioDataset("/scratch/users/sutd/nirmalen/cnndm/cnndm/diverse/val", args.model_type, is_test=True, max_len=512, is_sorted=False, max_num=args.max_num, total_len=args.total_len, is_pegasus=args.is_pegasus)
    if is_mp:
        train_sampler = torch.utils.data.distributed.DistributedSampler(
    	 train_set, num_replicas=world_size, rank=rank, shuffle=True)
        dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=False, num_workers=4, sampler=train_sampler)
        val_sampler = torch.utils.data.distributed.DistributedSampler(
    	 val_set, num_replicas=world_size, rank=rank)
        val_dataloader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=4, sampler=val_sampler)
        val_gen_dataloader = DataLoader(val_set, batch_size=8, shuffle=False, num_workers=4, sampler=val_sampler)
    else:
        dataloader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, num_workers=4)
        val_dataloader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=4)
        val_gen_dataloader = DataLoader(val_set, batch_size=8, shuffle=False, num_workers=4)
    # build models
    model_path = args.pretrained if args.pretrained is not None else args.model_type
    # candidate model path
    model = BRIO(model_path, tok.pad_token_id, is_pegasus=args.is_pegasus)
    chkpoint=torch.load("/scratch/users/sutd/nirmalen/BRIO/model_ranking.bin")
    model.load_state_dict(chkpoint)
    #candidate model
    # device = f"cuda:{args.gpuid}"
    mname = "facebook/bart-large-cnn"
    candidate_model = BartForConditionalGeneration.from_pretrained(mname)

    # lm_head_values = nn.Linear(512, 1, bias=False)
    linear=Linear()
    params = list(candidate_model.parameters()) + list(linear.parameters())
    candidate_optimizer = optim.Adam(params)
    ...
    if args.cuda:
        if is_mp:
            ...
            candidate_model = nn.parallel.DistributedDataParallel(candidate_model.to(gpuid), [gpuid], find_unused_parameters=False)
            linear=nn.parallel.DistributedDataParallel(linear.to(gpuid), [gpuid], find_unused_parameters=False)
        else:
            model = model.cuda()
    ...
    # s_optimizer.load_state_dict(chkpoint['opt'])
    if is_master:
    ...
    for epoch in range(args.epoch):
        ...
        for (i, batch) in enumerate(dataloader):
            ...
            step_cnt += 1
            # forward pass
            output = candidate_model.generate(
                        input_ids=batch["src_input_ids"],
                        attention_mask=batch["src_attention_mask"],
                        num_return_sequences=4, diversity_penalty=1.0, num_beams=4,num_beam_groups=4,
            ...
            for hypothesis in dec:
                hypothesis = hypothesis.replace("\n", " ")
                candidates.append(" ".join(hypothesis))
            rouge=[]
            ref=[tok.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in batch['tgt_ids']]
            ref = " ".join(ref).strip()    
            ref = process(ref)       
            for c in candidates:
                c = c.strip()
                ...
                rouge2 = score["rouge2"].fmeasure
                rougeLsum = score["rougeLsum"].fmeasure
                rouge.append((rouge1+rouge2+rougeLsum)/3)
            indices=np.flip(np.argsort(rouge))
            batch['candidate_ids']=torch.scatter_(1,indices,batch['candidate_ids'])
            batch['candidate_ids']=[batch[i]['tgt_ids']+ batch[i]['candidate_ids'] for i in range(len(batch))] 
            batch=collate_mp_brio(batch,tok.pad_token_id) 
