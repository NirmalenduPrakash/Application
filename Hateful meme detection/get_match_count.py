...
def process_text(text):
    ...
    tok_no_namedentities=[tok for tok in text_tokens if tok not in ents]
    tok_nva=[str(tok) for tok in document if tok.pos_ in ['VERB','NOUN','ADJ']]
    tok_nva.extend([x for tok in tok_nva for x in get_all_synonyms(tok)])
    return tok_nva

# match lists item by item
...
train_samples = pd.concat([train_samples,dev_samples])
...
# def count_match(row):    
    ...
    for crop in split_data[img_path.split('/')[-1]]:
        ...
        if(match_lists(text_tokens,tokenizer.tokenize(generated_text_prefix))>0):
            matched_count+=1
            break
# count_match_ = partial(count_match, row=row)
# Parallel(n_jobs=4)(delayed(count_match_)(row) for i,row in train_samples.iterrows())
print("match count:",matched_count/len(train_samples))                