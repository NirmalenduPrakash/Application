...
class ResNetVLBERTv2(Module):
    def __init__(self, config):

        super(ResNetVLBERTv2, self).__init__(config)
        ...
        if not config.NETWORK.BLIND:
            ...
            if config.NETWORK.VLBERT.object_word_embed_mode == 1:
                self.object_linguistic_embeddings = nn.Embedding(601, config.NETWORK.VLBERT.hidden_size)
        ...
        # self.mlp_stack = torch.nn.ModuleList([
        #     torch.nn.Sequential(
        ...
        if config.NETWORK.VLBERT.num_hidden_layers == 24:
            ...
            #     torch.tensor([
            #         -0.0001,  0.0005,  0.0019,  0.0020,  0.0020,  0.0018,  0.0026,  0.0019,
            #         0.0016,  0.0027,  0.0014,  0.0019,  0.0045,  0.0063,  0.0066,  0.0068,
            #         0.0091,  0.0124,  0.0164,  0.0118,  0.0104,  0.0102,  0.0114,  0.0177]),
            #     requires_grad=False)
            # self.gating = nn.Parameter(
            #     torch.tensor([
            #         0.0013, 0.0020, 0.0032, 0.0033, 0.0032, 0.0032, 0.0037, 0.0033, 0.0031,
            #         0.0041, 0.0033, 0.0041, 0.0064, 0.0080, 0.0082, 0.0085, 0.0104, 0.0130,
            #         0.0161, 0.0123, 0.0104, 0.0105, 0.0111, 0.0158]),
            #     requires_grad=True)
            self.gating = nn.Parameter(
                torch.tensor([
                    0.0044, 0.0050, 0.0056, 0.0058, 0.0057, 0.0055, 0.0059, 0.0057, 0.0058,
                    0.0065, 0.0062, 0.0072, 0.0089, 0.0098, 0.0100, 0.0102, 0.0113, 0.0126,
                    0.0139, 0.0123, 0.0112, 0.0112, 0.0112, 0.0129]),
                requires_grad=True)
        else:
            self.gating = nn.Parameter(
        ...
        if config.NETWORK.CLASSIFIER_TYPE == "2fc":
            self.final_mlp = torch.nn.Sequential(
                ...
                torch.nn.ReLU(inplace=True),
                torch.nn.Dropout(config.NETWORK.CLASSIFIER_DROPOUT, inplace=False),
                torch.nn.Linear(config.NETWORK.CLASSIFIER_HIDDEN_SIZE, config.NETWORK.CLASSIFIER_CLASS),
            )
            ...
            linear = nn.Linear(config.NETWORK.VLBERT.hidden_size, config.NETWORK.CLASSIFIER_CLASS)
    ...
    def train(self, mode=True):
        super(ResNetVLBERTv2, self).train(mode)
    ...
    def hidden_state_gate(self, hidden_states):
        hidden_states = [self.hidden_dropout(h[:, 0]) for h in hidden_states]
        all_hidden = torch.stack(hidden_states, dim=-1)
        # gating = F.softmax(self.gating)
        gating = torch.relu(self.gating)
        gating = gating / gating.sum()
        if self.train_steps % 200 == 0 and self.gating.requires_grad:
            print(self.gating)
            print(gating)
            print('-' * 100)
            
        gating = torch.unsqueeze(gating, 0)
        gating = torch.unsqueeze(gating, 0)
        # gating = torch.unsqueeze(gating, 0)
        _h = (all_hidden * gating).sum(-1)
        return _h
    ...
    def train_forward(self,
        ...
        # prepare text
        text_input_ids, text_token_type_ids, text_tags, text_mask = self.prepare_text(text_ids,
                                                                                    text_tags,
                                                                                    text_mask)
        ...
        _h = None
        # for li, mlp in enumerate(self.mlp_stack):
        #     l = (len(self.mlp_stack) - li) * 2
        #     h = hidden_states[-l][_batch_inds, 0]
        #     if _h is not None:
        #         _h = mlp(h + _h)
        #     else:
        #         _h = mlp(h)

        _h = self.hidden_state_gate(hidden_states)

        # for li, layer_h in enumerate(hidden_states):
        #     if _h is None:
        #         _h = layer_h[_batch_inds, 0] * gating[li]
        #     else:
        #         _h += layer_h[_batch_inds, 0] * gating[li]
        ###########################################

        # classifier
        logits = [self.final_mlp(_h) for _ in range(5)]
    ...
    def inference_forward(self,
        ...
        hm = self.hidden_state_gate(hidden_states)
