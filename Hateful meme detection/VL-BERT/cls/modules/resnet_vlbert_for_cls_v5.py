...
class ResNetVLBERTv5(Module):
    def __init__(self, config):
        ...
        if config.NETWORK.VLBERT.num_hidden_layers == 24:
            self.gating = nn.Parameter(
                torch.tensor([
                    0.0067, 0.0070, 0.0075, 0.0075, 0.0075, 0.0074, 0.0076, 0.0075, 0.0076,
                    0.0080, 0.0079, 0.0086, 0.0096, 0.0101, 0.0104, 0.0105, 0.0111, 0.0120,
                    0.0126, 0.0115, 0.0108, 0.0105, 0.0104, 0.0117]),
                requires_grad=True)
        else:
            self.gating = nn.Parameter(
    ...
    def train(self, mode=True):
        super(ResNetVLBERTv5, self).train(mode)
    ...
    def train_forward(self,
        ...
        _box_mask = (obj_and_img_boxes[:, :, 0] > - 1.5)  # NOTE: clip_pad_boxes(pad=-2)
        ...
        # loss
        ans_loss = [
            loss_fn(logit, label) * label.size(1)
            for logit in logits
        ]
        ans_loss = torch.stack(ans_loss, dim=-1).mean(dim=-1)

        outputs.update({
            'label_logits': torch.stack(logits, dim=-1).mean(dim=-1),
            'label': label,
            'ans_loss': ans_loss
        })
