...
class CLS3(Dataset):
    ...
    def __init__(self, root_path=None, image_set='train',
        ...
        self.fast_tokenizer = AutoTokenizer.from_pretrained(
            'bert-base-uncased', use_fast=True, return_offsets_mapping=True)
        ...
        if 'token_id' not in idb:
            ...
            if not self.random_drop_tags or (self.random_drop_tags and random.random() > 0.5):
                for p, img_tag in enumerate(img_tags):
                    if img_tag:
                        append_str = img_tag + (' [SEP] ' if p != len(img_tags) - 1 else '')
            ...
            for j, (race, gend) in enumerate(zip(race_tags, gender_tags)):
                if race is not None:
                    is_last = not any([rt is not None for rt in race_tags[j + 1:]])
            ...
            if self.use_img_box:
                ...
                text_partition += [0] * len(" [SEP] ") + person_tags_part  # additinoal partition id for [SEP]
    ...
    def load_annotations(self):
        ...
        train_json = (
            os.path.join(self.root_path, 'train.entity.aug.jsonl')
            if self.anno_aug 
            else os.path.join(self.root_path, 'train.entity.jsonl')
        )
        box_annos_json = os.path.join(self.root_path, 'box_annos.race_1.json')
        print("2")
        test_sample = []
        dev_sample = []
        train_sample = []
...
if __name__ == '__main__':
    ...
    with logger.catch(reraise=True):
        ...
        cls_data = CLS3(
            '/home/ron/Projects/VL-BERT/dataset/hateful_memes/',
